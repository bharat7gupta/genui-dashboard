...
export const filterSchema = z
  .object({
    start: z
      ...
      .optional(),
    status: z
      .enum(['Complete', 'Shipped', 'Processing', 'Cancelled', 'Returned', 'all'])
      .default('Complete'),
...
const dimensionSql = {
  ...
  country: 'country',
  category: 'category',
  brand: "coalesce(nullif(brand, ''), 'Unknown')",
...
const metricSql = {
  ...
  average_order_value: 'sum(sale_price) / nullif(count(distinct order_id), 0)',
};
...
export async function createDataStore(csvPath) {
  ...
  await connection.run(
    `CREATE TABLE sales AS SELECT * REPLACE (CAST(order_date AS DATE) AS order_date, CAST(sale_price AS DECIMAL(18, 4)) AS sale_price) FROM read_csv_auto('${file}')`,
  );
  const query = async (sql, params = []) => {
    const reader = await connection.runAndReadAll(sql, params);
    return JSON.parse(
      JSON.stringify(reader.getRowObjects(), (_, v) => (typeof v === 'bigint' ? Number(v) : v)),
    );
  };
  ...
  return {
    async metadata() {
      const [meta] = await query(
        "SELECT count(*) AS items, count(distinct order_id) AS orders, strftime(min(order_date), '%Y-%m-%d') AS start, strftime(max(order_date), '%Y-%m-%d') AS end FROM sales",
      );
      return {
        ...meta,
        countries: (await query('SELECT DISTINCT country FROM sales ORDER BY country')).map(
          (r) => r.country,
        ),
      };
    },
    async summary(filters) {
      ...
      const [row] = await query(
        `SELECT coalesce(sum(sale_price)::DOUBLE, 0) AS revenue, count(distinct order_id) AS orders, count(*) AS items, count(distinct user_id) AS customers, coalesce((sum(sale_price) / nullif(count(distinct order_id), 0))::DOUBLE, 0) AS average_order_value FROM sales ${w.sql}`,
        w.params,
      );
      return row;
    },
    async chart(input, filters) {
      ...
      if (spec.metric === 'average_daily_revenue') {
        ...
        const label =
          spec.dimension === 'day_of_week'
            ? "strftime(d, '%A')"
            : "CASE WHEN isodow(d) IN (6, 7) THEN 'Weekend' ELSE 'Weekday' END";
      ...
      const rows = await query(
        `SELECT ${dimensionSql[spec.dimension]} AS label, (${metricSql[spec.metric]})::DOUBLE AS value FROM sales ${w.sql} GROUP BY 1 ORDER BY ${order} LIMIT ${spec.limit}`,
        w.params,
      );

...
test('calendar averages account for unequal day counts, zero-sales days, filters and coverage', async () => {
  ...
  try {
    ...
    assert.deepEqual(
      result.rows.find((r) => r.label === 'Monday'),
      { label: 'Monday', value: 50, days: 2, total: 100 },
    );
    ...
    assert.deepEqual(types.rows, [
      { label: 'Weekday', value: 100 / 6, days: 6, total: 100 },
      { label: 'Weekend', value: 35, days: 2, total: 70 },
    ]);
    ...
    const extended = await store.chart(spec, {
      ...filters,
      start: '2024-01-01',
      end: '2026-01-01',
    });
    ...
    assert.throws(() =>
      compileDashboard({ title: 'Invalid average', charts: [{ ...spec, dimension: 'country' }] }),
    );
  } finally {
    store?.close();

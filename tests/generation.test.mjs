...
test('rejects unsupported metrics, invalid limits, invented data, and empty dashboards', () => {
  for (const change of [
    ...
    assert.throws(() =>
      compileDashboard({ ...dashboard, charts: [{ ...dashboard.charts[0], ...change }] }),
    );
  }
...
test('sends schema-constrained JSON, disables Qwen thinking, and preserves edit context', async (t) => {
  ...
  const result = await generateDashboard({
    prompt: 'Change that to a line chart',
    current: compileDashboard(dashboard).program,
    filters: { status: 'Complete', country: 'all' },
  ...
  assert.equal(
    request.format.anyOf[0].properties.charts.items.properties.metric.enum.includes('customers'),
    true,
  );
...
test('retries a malformed response once and validates the correction', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async () =>
    Response.json({ message: { content: ++calls === 1 ? '{broken' : JSON.stringify(dashboard) } }),
  );
  const result = await generateDashboard({
    prompt: 'Monthly customers',
    filters: {},
    base: 'http://localhost:11434',
    model: 'qwen3.5:4b',
  });
...
test('fails explicitly after two invalid responses instead of returning a fabricated dashboard', async (t) => {
  ...
  await assert.rejects(
    ...
    /current dashboard is unchanged/,
  );
  assert.equal(calls, 2);
});
    ...
    return Response.json({
      message: {
        content: JSON.stringify({
          unsupported: 'There are no timestamps in this CSV. Your dashboard is unchanged.',
        }),
      },
    });
  });
  const result = await generateDashboard({
    prompt: 'At what time of day do we have most sales?',
    current: compileDashboard(dashboard).program,
    filters: {},
    base: 'http://localhost:11434',
    model: 'qwen3.5:4b',
...
test('blocks a schema-valid weekday chart mislabeled as hourly sales', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async () =>
    Response.json({ message: { content: JSON.stringify(misleading) } }),
  );
  const result = await generateDashboard({
    prompt: 'Peak sales times?',
    filters: {},
    base: 'http://localhost:11434',
    model: 'qwen3.5:4b',
  });
